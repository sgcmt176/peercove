...
- **権限**: メモごとに「全体」のレベル(閲覧のみ / 閲覧 + 編集 / 見せない)と、
  **グループ単位**・メンバー個別の上書きを設定できます。優先順位は
  **メンバー個別 > グループ > 全体**(複数グループに属する場合は最も高い
  権限)。グループのメンバーを増減すると数秒で見え方も追従します。権限は
  表示名ではなくメンバー識別子(member_id)に紐付くため、改名しても維持
  されます。設定できるのは**所有者(作成者)とホスト管理者**です。
  メンバーを削除すると、その端末は以後共有メモを取得できません。
  (グループ指定は全端末が本バージョン以降のときに使えます。旧バージョンの
  メンバーが作ったグループは候補に出ません)
...
### リンク・コメント・リマインダー(M5 F-5)

- **共有ハブ**: ネットワーク詳細の「🗂 共有」に共有系機能がまとまります
  (現在は「メモ」。今後スケジュール表などはここにサブタブとして増えます)。
- **メモ間リンク**: 本文に `[[メモのタイトル]]` と書くとプレビューでリンクに
  なり、クリックでそのメモに移動できます(個人 → 個人、共有 → 共有)。
  存在しないタイトルはグレー表示。メモの下部には「バックリンク」
  (このメモを参照しているメモの一覧)が出ます。
- **チャットで共有メモを送る**: 共有メモの 🔗「リンクをコピー」で
  `@memo:<ID>` をコピーし、チャットに貼るだけ。受け取った側にはタイトル +
  抜粋のカードで表示され、クリックで該当メモが開きます。**閲覧権限のない
  相手には「アクセスできないメモ」とだけ表示され、内容は見えません**。
- **コメント**: 共有メモの閲覧画面下部で、閲覧できる人なら誰でもコメント
  できます(削除は本人・所有者・ホスト)。`@表示名` でメンションすると
  相手のデスクトップに通知が届きます(自分のメモへのコメントも通知)。
- **リマインダー**: メモの ⏰ から日時を指定すると、その時刻に通知が
  出ます(デスクトップ = アプリ起動中、Android = アプリを閉じていても可)。
  リマインダーは自分の端末だけのもので、他のメンバーには見えません。

## 共有スケジュール表(M6 G-1、ADR-0053)

「🗂 共有」→「📅 スケジュール」で、ネットワーク全員のカレンダーを共有
できます。

- **追加は誰でも**: タイトル・終日または開始/終了時刻・詳細を入力。
  数秒で全員のカレンダーに反映されます。
- **編集・削除は作成者とホスト**だけができます(他人の予定は閲覧のみ)。
  同じ予定を同時に編集した場合、後から保存した側は競合として拒否され、
  最新を読み込み直します。
- **オフライン**: メンバーはホスト未接続でも最後に同期した予定を閲覧
  できます(読み取り専用)。
- **チャットで共有**: 予定詳細の 🔗 で `@schedule:<ID>` をコピーして
  チャットに貼ると、日時つきカードで表示されます。
- 予定はホストの共有データベースに保存され、バックアップ(共有メモを
  含める)や接続診断の対象に自動で含まれます。共有メモ同様**平文保存**
  なので、秘密情報は書かないでください。

## 共有シート(M6 G-2、ADR-0054)

「🗂 共有」→「📊 表」で、Excel のような表をネットワーク全員で編集
できます。

- **セル単位の同時編集**: 別々のセルなら全員が同時に編集できます。同じ
  セルがぶつかった場合だけ、後から保存した側に「他の端末が先に更新
  しました」と表示され、最新値を確認して入れ直せます。
- **操作(デスクトップ)**: クリックで選択、入力で編集開始。Enter/Tab で
  確定して移動、Delete でクリア、Excel からのコピー範囲を Ctrl+V で
  貼り付けできます。Android はセルをタップして編集します。
- **シート管理**: シートは複数作れます(作成は誰でも、改名・削除は
  作成者とホスト)。
- **チャットで共有**: シートの ⋮ → 🔗 で `@sheet:<ID>` をコピーして
  チャットに貼るとカードで表示されます。
- **書式**(M6 H): 複数セルを選択(ドラッグ / Shift+クリック / Shift+矢印)
  してツールバーから太字・斜体・下線・取り消し線・フォントサイズ・
  文字色・背景色・配置・罫線を設定できます。セル結合、列幅・行高の
  ドラッグ変更、目盛線の表示切替、ウインドウ枠の固定にも対応。表は
  ダークモードでも白い紙面のままです。
- **操作**(デスクトップ): 右クリックで専用メニュー、Ctrl+F でシート内
  検索、Ctrl+Z で自分の直近操作の取り消し(他の人が先に変更したセルは
  競合として自動スキップ)。他のメンバーが選択中のセルは枠色 + 名前
  バッジで見えます。Android は書式・結合・列幅の表示と値の編集に対応。
- V1 の制限: 数式・マクロなし、行や列の挿入による繰り上げはありません。
  オフライン時は閲覧のみ。予定・メモと同じく**平文保存**です。

### メンション・通知(M6 H)

- チャットとメモのコメントの両方で `@` からメンバーをサジェスト入力
  できます。`@All` で全員宛。メンションされた人・自分のメモにコメントが
  付いた人には、OS 通知に加えて**チャットにお知らせ行**(クリックで該当
  メモへ飛べるカード付き)が届きます。
- スケジュールは土曜が青、日曜・祝日が赤で表示され、祝日名も出ます
  (祝日データは holidays-jp から取得)。予定には参加メンバーの設定と
  「自分の予定」フィルタ、**予定ごとのリマインダー**(5 分前〜1 日前の
  プリセット + 任意日時、複数可)があります。メモのリマインダーは
  スケジュールへ移設しました。
