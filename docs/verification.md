...
99. **個人メモ → 共有メモ**: 個人メモの 📤「共有メモへコピー」で、接続中の
    ...
    同様にコピーできること。
100. **スループット実機計測(C-2 / C-3 Phase 1、ADR-0050)**: 実機(VM で
    ない Windows 2〜3 台)を本バージョンへ更新し、iperf3 で計測して結果を
    報告する(Phase 2 = 並列化に進むかの判断材料)。
    1. 直接経路: member で `iperf3 -s`、host から `iperf3 -c <memberの仮想IP>`
       (TCP)。`-R` で逆方向も。
    2. リレー経路: member A ↔ member B(直接通信 OFF で計測すると確実に
       ホスト経由になる)。
    3. 参考: 開発機のローカルループバック上限は direct ~1.7 Gbps /
       relay ~1.4 Gbps(ADR-0050)。実測がこれよりずっと低ければ回線・
       NIC・MTU 側、近ければユーザー空間 CPU が次のボトルネック。
