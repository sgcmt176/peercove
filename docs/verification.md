...
### チャット通知(Android、VPN 接続中のみ)

41. **新着通知**: アプリを閉じた(バックグラウンドの)状態でチャットを
    受信すると、会話ごとにまとまった通知(送信者と本文)が出ること。
    アプリを開いている間は通知が出ない(画面の未読バッジで見える)こと。
42. **通知からの操作**: 通知の「返信」でその場で返信でき、相手に届くこと。
    「既読にする」で通知が消え、アプリ側の未読バッジも消えること。
    通知本体のタップでその会話が直接開くこと。
    アプリ内で会話を開いて読むと、出ていた通知が数秒で自動で消えること。

### チャットの再送キュー(Android)

43. **バナー通知**: アプリを閉じた状態でチャットを受信すると、画面上部に
    バナー(ヘッズアップ)+ 音で通知されること(手順 41 の強化)。
44. **自動再送**: ホストを止めた(または未同期の)状態でチャットを送ると、
    エラーにならず吹き出しに「未送信(自動再送します)」と出ること。
    ホストが復帰すると**操作なしで**自動的に届き、表示が通常に戻ること。
    相手側で二重に受信されないこと(同じ ID は受信側が弾く)。
45. **手動再送・取消**: 未送信の吹き出しの「取消」で自動再送が止まり
    「送信できませんでした」になること。「再送」でもう一度送れること。

### 送信キューの永続化(Android)+ デスクトップの送信キュー

53. **再起動を跨ぐ自動再送(Android)**: ホストを止めた状態でチャットを送り
    「未送信(自動再送します)」のまま**アプリを強制終了 → 起動し直して接続**
    → 手動操作なしで「送信中…」に戻り、ホスト復帰後に自動で届くこと。
54. **デスクトップの自動再送**: メンバー(またはホスト)の PC で、相手が
    オフラインの相手に 1:1 チャットを送る → エラーにならず吹き出しに
    「未送信(自動再送します)」と出ること。相手がオンラインに戻ると
    **操作なしで**自動的に届き、表示が通常に戻ること。二重受信しないこと。
55. **デスクトップの取消・再送**: 未送信の吹き出しの「取消」で自動再送が
    止まり「送信失敗」+「再送」になること。「再送」でキューに戻ること。
    デーモン再起動(サービス再起動)後も未送達分の自動再送が続くこと
    (`networks/<net>.chatq.json` に保存)。
56. **ファイル添付の扱い**: オフラインの相手にはテキストは送れる(キュー)が、
    📎(ファイル)はグレーのまま送れないこと(ファイルは即時転送のみ)。

### 通知の秘匿とミュート(Android)

46. **ロック画面で本文を隠す**: 設定タブ末尾「通知」の「ロック画面で本文を
    隠す」を ON → アプリを閉じてロックした状態でチャットを受信すると、
    ロック画面には「新着メッセージがあります」だけが出て、ロック解除後の
    通知シェードでは本文が読めること。OFF に戻すとロック画面でも本文が
    出ること。
47. **会話単位ミュート**: トーク一覧で会話を**長押し → 「通知をミュート」**
    → 行に 🔕 が付き、その会話のチャットを受信しても通知が出ない
    (未読バッジは付く)こと。他の会話の通知は通常どおり出ること。
    長押し → 「ミュートを解除」で通知が復活すること。ミュートはアプリを
    再起動しても保持されること。

### 受信ファイルの開く・共有(Android)

48. **操作シート**: トークのファイル/画像バブルをタップ → 下からシートが
    開き、ファイル名・サイズと「開く / 共有 / ダウンロード(PeerCove)へ
    保存 / 名前を付けて保存 / SHA-256 をコピー」が並ぶこと
    (自分が送ったファイルのバブルでも開けること)。
49. **開く**: 画像 → フォトビューア、PDF 等 → 対応アプリで直接開けること。
    対応アプリが無い形式は「開けませんでした」と出ること。
50. **共有**: 「共有」→ OS の共有シートから他アプリ(メール等)へ渡せること。
51. **名前を付けて保存**: 保存先とファイル名を選んで保存でき、選んだ場所に
    実体があること。
52. **SHA-256**: 「SHA-256 をコピー」でハッシュがクリップボードに入り、
    デスクトップ側(受信ボックスの詳細等)や `certutil -hashfile` の値と
    一致すること。
...
40. **ピン留め**: ピン留めした会話は 📌 付きで**常に一番上**に表示され、
    ...
    - Android: 会話行を**長押し → メニューの「ピン留め」**で付け外し

### グループ管理(Android)

57. **改名**: グループの会話を開き、右上の ⚙ →「グループの管理」で名前を
    変えて保存 → 自分とデスクトップ側の両方でグループ名が変わり、
    「グループ名が「旧」から「新」に変わりました」のお知らせが載ること。
58. **メンバー追加**: 同じダイアログでメンバーを選んで保存 → 追加された
    相手のトーク一覧にグループが現れること(「〜 が参加しました」)。
59. **退出**: ダイアログの「このグループから退出する…」→ 確認 → トーク
    一覧からグループが消え、残ったメンバー側に「〜 が退出しました」が
    載ること。履歴はスマホに残る(会話一覧からは消える)。
60. **全員オフライン時**: メンバーが誰もオンラインでない状態で改名すると
    「オンラインのメンバーに届けられませんでした…」とエラーになり、
    変更が適用されないこと(あとでやり直す)。

### 通信品質履歴(Android)

61. **RTT グラフ**: 接続して数分置いた後、設定タブの「通信品質」に RTT の
    スパークライン(直近 6 時間)が描かれること(30 秒ごとに記録)。
62. **イベント履歴**: 接続・切断・回線切替(Wi-Fi ↔ モバイル)・張り直しが
    「イベント履歴」に時刻付きで並ぶこと。切断 → 接続し直しでも記録が
    消えないこと(端末ローカルに保存)。
63. **品質低下通知**: 通信が悪化した状態(RTT 500ms 超 or 送信失敗)が
    30 秒続くと「通信品質が低下しています」の通知が 1 回出て、回復すると
    自動で消えること。イベント履歴にも低下/回復が載ること。

### チャット検索・ファイル一覧・ストレージ管理(Android)

64. **検索**: トークタブ上部の「メッセージを検索」に語句を入れると、全会話
    横断でヒットが新しい順に並び(会話名・時刻付き)、タップでその会話が
    開くこと。ファイル名でもヒットすること。
65. **ファイル一覧**: 検索バー右の 📎 → 送受信ファイルが新しい順に並び、
    タップで開く/共有/保存の操作シートが出ること(実体が消えたものは
    「実体なし」でタップ不可)。
66. **ストレージ管理**: 設定タブ末尾に受信ボックス/送信キャッシュ/
    チャット履歴/品質履歴のサイズが出ること。
    - 「受信ボックスを空にする」→ 確認後にサイズが 0 になり、既存の
      画像バブルは開けなくなる(履歴の行は残る)こと
    - 「チャット履歴を削除」→ 確認後に全会話が空になり、
      「チャット履歴を削除しました」の 1 行だけ残ること(接続中のみ。
      相手側の履歴はそのまま)
    - 送信キャッシュ/品質履歴の削除でサイズが 0 になること

### ファイル転送の中断再開(デスクトップ同士、ADR-0045)

67. **再開**: 大きめのファイル(数百 MB 推奨)をメンバーへ送信中に、受信側の
    接続を切る(Wi-Fi オフ等)→ 送信は失敗し、受信側の受信ボックスに
    `<名前>.part` と `<名前>.pcvresume` が残ること。回線を戻して**同じ
    ファイルをもう一度送る**と、進捗が途中から始まり(ログ「書きかけから
    再開します(n/m バイト)」)、完了後のファイルが元と一致すること
    (SHA-256 はファイル全体で検証される)。
68. **壊れた書きかけ**: `.part` を手で書き換えてから再送すると、チェックサム
    不一致で失敗し、`.part` が捨てられること。もう一度送ると最初から
    転送されて成功すること。
69. **旧バージョン互換**: 旧バージョンの相手との送受信は従来どおり
    (再開なしで最初から)動くこと。

### 端末バッジ(ADR-0046)

70. **OS バッジ**: メンバーが接続すると、ホスト(および他メンバー)の
    メンバー一覧に、バージョンの隣へ **Windows / Linux / Android** の
    バッジが出ること(デスクトップ UI・スマホのメンバータブの両方)。
    オフラインのメンバー・旧バージョンの相手はバッジなしのまま。

### v0.1.1 検証フィードバック対応

71. **ロック画面の本文秘匿(修正)**: 端末の「ロック画面: すべて表示」設定でも、
    設定タブの「ロック画面で本文を隠す」を ON にすると、**ロック中**に届いた
    チャット通知が本文を出さず「新着メッセージがあります」だけになること。
    解錠すると数秒以内に本文入り(送信者・本文)へ差し替わること。OFF なら
    ロック中でも本文が出ること。
72. **デスクトップのチャット検索**: チャットタブ左上の検索ボックスに語句を入れる
    と、全会話横断のヒットが新しい順に並び(会話名 + 本文スニペット)、
    クリックでその会話が開くこと。ファイル名でもヒットすること。
73. **デスクトップの右クリックメニュー**: 会話行を右クリック →「ピン留め」
    「通知をミュート」ができること。ミュート会話は OS 通知が出ない
    (🔕 が付く)こと。
74. **ピン留めの並び(両プラットフォーム)**: ピン留めした会話は、送受信が
    あっても**ピン内で順番が入れ替わらない**こと(新規ピンは最下部に付く)。
    - デスクトップ: 右クリック →「上に移動」「下に移動」で並べ替え
    - Android: 会話を長押し →「上に移動」「下に移動」で並べ替え
    並び順はアプリ再起動後も保たれること。
75. **メンバー再追加時の履歴クリア**: メンバー(例 member-3)を削除 → 同名で
    再度招待・参加させると、その相手との 1:1 履歴が**空になっている**こと
    (別人=別の鍵として扱う)。ホスト側・他メンバー側の両方で。接続したまま
    でも数秒(台帳同期の周期)で手元の表示から消えること。
76. **クリップボード画像の貼り付け送信(デスクトップ)**: スクリーンショット
    (Win+Shift+S 等)をクリップボードに取ってから、チャットの入力欄で
    **Ctrl+V** → ドラッグ&ドロップと同じ送信確認モーダルが開き、送ると相手に
    画像として届くこと(相手のバブルにサムネイル表示)。テキストの貼り付けは
    従来どおり文字が入ること。オフライン/退出中の相手には送れないこと。

### v0.1.2 検証フィードバック対応(2026-07-20 夜、ADR-0047)

前提: 全端末(ホスト・メンバー PC・Android)を v0.1.2 に更新して接続し直す
(台帳の `member_id` はホストが配るので、**ホストの更新が必須**)。

77. **Android の通知が復活**: VPN 接続中にアプリを閉じた状態でチャットを
    受信すると通知が出ること(ロック中・解錠中とも。ミュートなし・
    「ロック画面で本文を隠す」OFF の会話で確認)。会話を開いたまま
    ホーム/ロックへ逃げた場合も、その会話の新着で通知が出ること。
    ネットワークを削除 → 同じ招待元へ参加し直した後も通知が出ること
    (既読位置の自己修復)。
78. **鍵ローテーションで履歴が消えない**: Android を新規参加させる(参加
    直後に自動で鍵が更新される)→ 他の端末でその相手との 1:1 履歴が
    **消えない**こと(v0.1.1 では公開鍵比較のため消えていた)。
    デスクトップの「鍵を更新」でも同様。
79. **再追加で送信待ちも破棄**: メンバーを**切断(オフライン)**させてから
    そのメンバー宛に 1:1 チャットを送り「送信中(再送待ち)」でためる →
    ホストからそのメンバーを削除 → 同名・同 DNS 名で再度招待・参加させる →
    たまっていたメッセージが**新しく参加したメンバーに配送されない**こと。
    送信側の履歴では送信中の表示が消え、1:1 履歴もクリアされて「〜は新しい
    端末として参加しました」のお知らせが載る。
    (2 段で防御: 台帳同期での送信待ち破棄 + 配送直前の member_id 照合。
    再追加を素早く行っても新メンバーには届かない)
80. **メンバー削除の通知と自動整理**: グループに入っているメンバーをホストが
    削除すると、数秒〜数十秒で
    - 各メンバーの全体会話に「〜がネットワークから削除されました」が載る
    - そのグループから削除されたメンバーが自動で外れ、グループ会話に
      「〜がグループのメンバーから外れました」が載る
    こと。削除されたメンバーを**再追加してもグループには戻らない**
    (必要ならグループ管理で追加し直す)こと。
81. **グループのキック**: グループ管理(デスクトップ = チャットの ⚙、
    Android = 会話の ⚙ →「グループの管理」)に「メンバーを外す」が並び、
    選んで保存すると
    - 残ったメンバーに「〜がグループのメンバーから外れました」が載る
    - **外された本人**(オンライン時)のトーク一覧からグループが消え、
      「あなたはグループのメンバーから外れました」が載る
    こと。自分は外せない(「退出」を使う)こと。
82. **ファイルの貼り付け送信(デスクトップ)**: エクスプローラーでファイル
    (画像以外も可)を **Ctrl+C** → チャット入力欄で **Ctrl+V** → 元の
    ファイル名のまま送信確認モーダルが開き、送れること。複数ファイルの
    同時コピーも可。スクリーンショットの貼り付け(手順 76)も従来どおり
    動くこと。
83. **スマホから全体/グループへファイル送信**: Android で「全体」または
    グループの会話を開くと **+(添付)ボタン**が出て、送るとオンラインの
    対象メンバー全員に届くこと(履歴には 1 エントリ。誰か 1 人でも届けば
    成功)。全員オフラインなら「オンラインの宛先がいません…」と出ること。
84. **ネットワーク削除の確認(Android)**: ネットワーク一覧の削除ボタンを
    押すと確認ダイアログが出て、「キャンセル」なら何も起きず、「削除」で
    初めて一覧から消えること。
85. **メンバーによる招待発行(ADR-0048)**: 前提 = ホスト・メンバーとも
    本バージョン。
    1. ホストのメンバー一覧で対象メンバーの **ℹ(詳細)** を開き、
       「この端末にメンバー招待の発行を許可」を ON にする。
    2. 数秒後、そのメンバー(デスクトップ = メンバー一覧右上、Android =
       メンバータブ右上)に「招待を発行」ボタンが現れること。デスクトップ・
       Android どちらの端末でも同様に使えること。
    3. 発行フォームは名前(省略可)と有効期限(1 時間 / 1 日 / 7 日。無期限
       なし)のみ。発行するとトークン + QR が表示され、コピー・参加リンク
       (Android は共有シート)で渡せること。
    4. **可視化**: 各メンバーの「全体」会話に「〜がメンバー招待を発行しました
       (〜)」のお知らせ行が載ること。ホストのログに発行者 IP と invite_id
       が残り、**トークンや名前はログに出ない**こと。
    5. 発行されたトークンで新しい端末が参加できること。参加後、その端末の
       メンバー詳細の「招待者」に発行したメンバーの名前が出ること
       (ホスト発行の既存メンバーは「ホスト」)。
86. **招待発行の権限 2 段**: ホストの設定「メンバーによる招待発行を許可」を
    OFF → 指名済み端末でも数秒でボタンが消え、直前に開いていたフォームから
    発行しても「ホストの設定で〜無効になっています」と拒否されること。
    ON に戻し、メンバー詳細の指名を OFF にした場合も同様にボタンが消えること。
87. **メンバー詳細ページ**: メンバー一覧の ℹ で、表示名・役割・仮想 IP・
    DNS 名・OS・アプリバージョン・公開鍵(デスクトップのみ)・状態・経路・
    招待状態・**招待者**が確認できること(デスクトップ + Android)。
88. **個人メモ(デスクトップ、M5 F-1 / ADR-0049)**: 前提 = デーモンも
    本バージョンへ更新済み(旧デーモンだと「メモを読み込めません」)。
    サイドバーの「📝 メモ」で:
    1. **作成・自動保存**: 「＋ 新規メモ」→ タイトル・本文を入力すると
       操作なしで「保存済み」になること。UI を閉じて開き直しても、
       **VPN 未接続(全ネットワーク停止中)でも**残っていること。
    2. **Markdown**: 見出し・太字・チェックリスト・表・コードブロック等を
       書いてプレビュー / 分割表示で整形されること。プレビューの
       チェックボックスをクリックすると本文の `[ ]` ⇔ `[x]` が切り替わる
       こと。リンクは既定ブラウザで開くこと。
    3. **整理**: フォルダー作成 → メモを移動 → 絞り込み。フォルダー削除で
       中のメモが「フォルダーなし」に残ること。タグ(カンマ区切り)を付けて
       タグで絞り込めること。ピン留めが一覧の先頭に固定されること。
       アーカイブしたメモが「アーカイブ」タブに移ること。
    4. **検索**: タイトル・本文の一部(日本語 3 文字以上と 2 文字の両方)で
       検索がヒットすること。
    5. **ゴミ箱**: ゴミ箱へ移動 → 一覧から消える → ゴミ箱タブで復元 /
       完全削除(確認あり)/ 空にできること。ゴミ箱のメモは読み取り専用。
    6. **txt 入出力**: メモを「テキストへ保存」で `タイトル.txt` に保存 →
       中身が本文そのままであること。「テキスト取り込み」で複数の .txt を
       選ぶと、ファイル名がタイトルのメモとして取り込まれること。
    7. **上限**: 256 KiB 超の本文を貼り付けると「本文が大きすぎます…」と
       理由が表示されること(黙って失敗しない)。
89. **個人メモ(Android)**: ホーム右上の 📝 アイコンから同様に
    作成・自動保存・プレビュー(Markdown)・フォルダー・タグ・ピン留め・
    アーカイブ・ゴミ箱・検索が使えること。メモの ⋮ メニューの
    「テキストへ保存」で保存先を選んで .txt に書き出し、一覧の
    「テキスト取り込み」で .txt を選んで取り込めること。
    機内モード(VPN 未接続)でも個人メモの作成・編集ができること。
    デスクトップとは独立した端末内データであること(同期されない)。
    (2026-07-21 FB 対応)プレビューに切り替えてもアプリが落ちないこと。
    検索は「めんて」でカタカナの「メンテ」がヒットする(逆も)こと。
90. **共有メモの基本フロー(M5 F-2 / ADR-0049)**: 前提 = ホスト・メンバー
    とも本バージョン(旧ホストだと「同期できていません」表示のまま)。
    1. ネットワーク詳細のサイドバー「📝 メモ」(Android = メモタブ)を開く。
       平文保存の注意書きが表示されること。
    2. ホストで「＋ 新規メモ」→ タイトル・本文を入力 → 自動保存。
       **数秒以内に**接続中のメンバー(デスクトップ + Android)の一覧・
       閲覧画面に反映されること(リアルタイム閲覧)。
    3. メンバーで「編集」→ 編集ロックを取得して編集 → 他の端末の一覧に
       「〜が編集中」が出て、その間は他の人の「編集」ボタンが押せないこと。
       「編集を終了」でロックが外れ、他の端末からも表示が消えること。
    4. 編集中のメンバーの端末を切断(トンネル切断)→ 数秒でロックが自動
       解放されること。何もせず放置した場合も 2 分で解放されること。
       ホストは「ロックを強制解除」でも外せること。
    5. メモを「個人メモとしてコピー」すると個人メモに複製されること。
       「テキストへ保存」で .txt に書き出せること。
91. **共有メモの権限**: ホスト(または所有者)がメモの 🔑 で
    - 全体 = 「閲覧のみ」(既定)→ 他メンバーは読めるが「編集」が
      押せないこと(一覧に「閲覧のみ」バッジ)。
    - 特定メンバーだけ「閲覧 + 編集」→ そのメンバーだけ編集できること。
    - 特定メンバーを「見せない」→ そのメンバーの一覧から**数秒で消える**
      こと(キャッシュからも消え、オフラインでも見えない)。
    - メンバーが作ったメモは作成者が所有者となり、権限変更・削除が
      できること。他メンバーのメモは削除できないこと。
92. **共有メモのオフライン閲覧**: メンバーで共有メモを受信した状態で
    トンネルを切断 → メモ一覧・本文は引き続き閲覧できるが
    「オフライン…読み取り専用」と表示され、新規作成・編集ができないこと。
    再接続すると自動で同期され、切断中にホスト側で変わった内容・削除が
    反映されること。
93. **共有メモの競合と検索**: 2 台で同じメモを編集しようとすると後の 1 台は
    「〜が編集中です」と拒否されること(編集は 1 人ずつ)。共有メモの検索も
    ひらがな/カタカナを同一視すること。ゴミ箱(ホスト・所有者)から復元・
    完全削除ができること。
94. **共有メモの変更履歴(M5 F-3 / ADR-0049)**: デスクトップの共有メモで
    1. 編集 → 内容を変えて「編集を終了」→ 「履歴」に「編集終了」の版が
       残ること(内容を変えずに終了した場合は増えない)。
    2. 編集中に「版を保存」→ 「手動」の版が残ること(同じ内容のまま
       押し直しても増えないこと)。
    3. 履歴から版を選ぶと当時の本文が読めること。「現在と比較」で追加行
       (+ 緑)・削除行(- 赤)の差分が表示されること。
    4. 履歴の「個人メモへコピー」で当時の内容が個人メモに複製されること。
    5. Android でも閲覧画面の履歴アイコンから一覧・本文・差分が見られること。
95. **履歴からの復元**: 「この版へ復元」(確認あり)→ 本文が置き換わり、
    復元前の内容が「復元前」として履歴に残ること。他の端末にも数秒で
    反映されること。他のメンバーが編集ロック中は「〜が編集中です」と
    拒否されること。閲覧のみのメンバーには復元操作が出ないこと。
    Android(編集権限あり)でも復元できること。
96. **共有メモの上限設定(ホスト)**: 共有メモの「上限設定」で件数上限を
    最小値にする → 超える新規作成が「件数が上限…」と理由つきで拒否される
    こと(黙って失敗しない)。本文上限を 1 KiB にすると長文の保存が拒否
    されること。値を既定に戻すと再び保存できること。メンバー側には
    上限設定ボタンが出ないこと。
97. **バックアップへの同梱(ADR-0034 改訂)**: ホストのバックアップ作成で
    「共有メモを含める」を ON → 復元プレビューの内訳に「共有メモ」が出て、
    別名で復元するとメモ・履歴・権限ごと復元されること(復元先で共有メモを
    開いて確認)。OFF で作ったバックアップには含まれないこと。
98. **メモの診断**: 共有メモを使った状態で接続診断を実行 → ホストは共有メモ
    DB のチェック(容量・件数)、メンバーはキャッシュサイズのチェックが
    「正常」で表示されること。共有メモを一度も使っていないネットワークでは
    この項目が出ないこと。
99. **個人メモ → 共有メモ**: 個人メモの 📤「共有メモへコピー」で、接続中の
    ネットワークの共有メモとして作成されること(共有メモ対応ネットワークに
    複数接続中は選択ダイアログ)。編集権限のないメンバーからは「権限が
    ありません」と拒否されること。Android でも個人メモの ⋮ メニューから
    同様にコピーできること。
100. **スループット実機計測(C-2 / C-3 Phase 1、ADR-0050)**: ✅ 実測済み
    (2026-07-23。実機間直接 306 Mbps・リレー 350 Mbps・同一 PC 内
    958 Mbps・素の Wi-Fi 514 Mbps。結果と結論は ADR-0050)。
101. **再作成必須設定の自動反映**: 接続中のネットワークの設定画面で
    1. メンバーの「待受ポート」または「MTU」を変更して保存 → 「自動で
       再接続して反映」の案内が出て、**手動の切断なしに**約 5〜10 秒で
       再接続されること(ログに「設定変更(…)を検知したためトンネルを
       作り直します」)。接続先(ホストの IP:ポート)の変更も同様。
    2. ホストの「待受ポート」変更も同様に自動で作り直されること。UPnP
       使用時はログにポート開放の張り直しが出て、メンバーが新ポートへ
       再接続できること。
    3. 「直接通信」「受信ファイルの上限」は従来どおり再接続なしで約 5 秒で
       反映されること(再接続の案内が出ない)。
102. **共有メモのグループ権限(M5 F-4 / ADR-0051)**: 前提 = 全端末とも
    本バージョン(旧メンバーが作ったグループはホストに届かず候補に出ない)。
    1. メンバー A がチャットでグループ(A・B のみ、ホストを含めない)を作る
       → 数秒〜30 秒でホストの `<config>.groups.json` にそのグループが載る
       こと。**ホストのチャット画面には出ない**こと。
    2. ホストで共有メモを作り、権限 🔑 で「全体 = 見せない」+ グループ段で
       そのグループに「閲覧のみ」→ A・B には見え、C(グループ外)には
       見えないこと。「閲覧 + 編集」にすると A・B が編集できること。
    3. グループに C を追加 → 数秒で C にもメモが現れること。C を外す →
       数秒で C の一覧から消えること(キャッシュからも)。
    4. メンバー個別の指定が優先されること: グループ「編集」+ B 個別
       「見せない」→ B には見えないこと。
    5. グループを改名 → 権限ダイアログの表示名が追従すること。グループを
       削除(全員退出)しても権限指定が残り、「(不明なグループ)」として
       解除できること。
    6. Android(グループ所属メンバー)でも 2〜3 の見え方が同じであること
       (権限の編集 UI は Android には無い = 仕様)。
103. **チャットの共有メモリンクカード `@memo:id`(M5 F-5 Stage 4、ADR-0052
    決定 1)**:
    1. 共有メモの閲覧画面(🔗 リンクをコピー)でリンクをコピーし、チャット
       に貼り付けて送信 → 送信直後は自分の画面にも 📝 アイコン + タイトル +
       抜粋 1 行のカードで表示されること(生の `@memo:xxxx` 文字列のままで
       はないこと)。
    2. 受信側(閲覧権限あり)でも同じ内容のカードが表示され、クリック/
       タップで「共有」ハブが開き、該当メモが選択された状態になること。
    3. 受信側にそのメモの閲覧権限が無い(または削除済み)場合は「🔒
       アクセスできないメモ」カードになり、タイトル・抜粋は一切表示され
       ないこと。
    4. `@memo:xxxx` の前後に通常の文章や URL を混ぜたメッセージでも、
       トークン部分だけがカード化され、それ以外の文章・URL リンク化は
       従来どおり動くこと。
    5. 存在しない ID(`@memo:0000`)は「🔒 アクセスできないメモ」カードに
       なり、未登録の種別(`@schedule:xxxx`)はただの文字列として表示される
       (カード化されない)こと。
    6. Android でも 1〜3 と同様(共有メモ閲覧画面の🔗リンクアイコンでコピー
       → チャットのカード表示 → タップで「共有」タブの該当メモへ)。
104. **メモ間リンクとバックリンク(M5 F-5、ADR-0052 決定 2)**: 個人メモ・
    共有メモそれぞれで
    1. メモ A の本文に `[[メモBのタイトル]]` と書く → プレビューでリンクに
       なり、クリック/タップでメモ B が開くこと。存在しないタイトルは
       グレー表示になり、クリックすると「見つかりません」と出ること。
    2. メモ B を開くと下部に「バックリンク (1)」が出て、クリックでメモ A に
       戻れること。リンクしているメモが無ければ欄自体が出ないこと。
    3. 共有メモではオフライン(トンネル切断)中もリンク解決・バックリンクが
       動くこと(キャッシュ範囲)。
    4. ネットワーク詳細のサイドバーが「🗂 共有」になっており、中のサブタブ
       「メモ」に従来の共有メモがあること(Android = 「共有」タブ)。
105. **共有メモのコメントとメンション(M5 F-5、ADR-0052 決定 4・5)**:
    1. 閲覧のみのメンバーでもコメントを書けること。一覧に 💬1 が付き、
       他の端末にも数秒で反映されること。
    2. 自分のコメントは削除できる。他人のコメントは所有者・ホストだけが
       削除できること。
    3. デスクトップで `@` を入力するとメンバー名がサジェストされること。
       `@<相手の表示名>` を含むコメントを書くと、**相手のデスクトップに
       OS 通知**が出ること。自分が所有するメモに他人がコメントした時も
       所有者に通知が出ること(自分のコメントでは通知されない)。
    4. 4KiB 超の本文は理由つきで拒否されること。
    5. Android でもコメントの閲覧・追加・削除ができること(OS 通知は
       Android 未対応 = 既知の制限)。
106. **メモのリマインダー(M5 F-5、ADR-0052 決定 6)**: 個人メモ・共有メモ
    それぞれで
    1. ⏰ からリマインダー日時を設定(過去日時は拒否)。設定済みは ⏰ 表示で
       分かり、解除もできること。
    2. デスクトップ: 指定時刻(±30 秒程度)に OS 通知「メモのタイトル」が
       出ること(UI 起動中)。
    3. Android: 指定時刻前後に通知が出て、タップで該当メモが開くこと
       (アプリを閉じていても・VPN 未接続でも発火する。数分の誤差は許容 =
       省電力設計)。端末を再起動しても未発火のリマインダーが生きている
       こと。
    4. リマインダーは端末ローカルであること(他のメンバーには見えない)。
107. **共有スケジュール表(M6 G-1、ADR-0053)**: 前提 = 全端末とも
    本バージョン。「🗂 共有」→「📅 スケジュール」で
    1. **追加と反映**: どのメンバーでも予定を追加でき(タイトル・終日/
       時刻・詳細)、数秒で他の端末のカレンダーにも出ること
       (デスクトップ = セルにタイトル、Android = ドット + 件数)。
    2. **権限**: 自分の予定は編集・削除できるが、他人の予定にはボタンが
       出ないこと。ホストは全予定を編集・削除できること。
    3. **競合**: 2 台で同じ予定を同時に編集 → 後から保存した側が「他の
       端末の変更が先に保存されています」と拒否され、再取得で最新に
       なること。
    4. **オフライン**: メンバーがトンネル切断中もカレンダーを閲覧できる
       (読み取り専用表示)こと。再接続で自動同期されること。
    5. **チャットカード**: 予定詳細の 🔗 で `@schedule:<id>` をコピーして
       チャットへ貼る → 📅 + タイトル + 日時のカードで表示され、クリック/
       タップでスケジュールの該当予定が開くこと。
    6. **バックアップ/診断**: ホストのバックアップ(共有メモを含める)に
       予定も含まれて復元できること。診断の共有メモ DB チェックに
       schedule_count が出ること。
108. **共有シート(M6 G-2、ADR-0054)**: 「🗂 共有」→「📊 表」で
    1. **作成とセル編集**: シートを作成(誰でも可)し、セルをクリック
       (Android = タップ)して値を入力 → 数秒で他の端末にも反映される
       こと。数値は右寄せで表示されること。
    2. **同時編集**: 2 台で**別々のセル**を同時に編集 → 両方とも保存される
       こと。**同じセル**を同時に編集 → 後から保存した側が「他の端末が
       先に更新した」と通知され、セルが最新値(黄色ハイライト)になる
       こと。
    3. **キーボード操作(デスクトップ)**: 矢印で移動、Enter/Tab で確定 +
       移動、Delete でセルクリア、Excel からコピーした範囲の Ctrl+V
       貼り付けができること。
    4. **シート管理**: 改名・削除(確認あり)は作成者とホストだけができる
       こと(他人のシートにはメニューが出ない)。
    5. **チャットカード**: ⋮ の 🔗 で `@sheet:<id>` をコピーしてチャットへ
       → 📊 カードで表示され、クリックで該当シートが開くこと。
    6. **オフライン**: メンバーが切断中もシートを閲覧できること(読み取り
       専用)。再接続で自動同期。
109. **情報共有スイート改修の Android 追随(M6 H-6、ADR-0055)**: デスクトップで
    設定した内容が Android 側の表示にも反映されることを確認する。
    1. **曜日色・祝日**: 「📅 スケジュール」のカレンダーで土曜の日付が青、
       日曜が赤で表示されること。祝日(取得できていれば)も赤 + 選択した
       日の見出しに祝日名が出ること(通信できない環境でも週末色だけで
       落ちずに動作すること)。
    2. **参加メンバー**: 予定の追加/編集画面でメンバーを複数選択でき、
       詳細シートに参加者名のバッジ、日別リストに 👥件数 バッジが出る
       こと。「すべて/自分」フィルタで、自分が作成者または参加者の予定
       だけに絞れること。
    3. **複数リマインダー**: 予定詳細でプリセット(5分前/15分前/30分前/
       1時間前/1日前)または任意日時のリマインダーを複数件設定でき、
       一覧からそれぞれ個別に削除できること。同じ予定に複数件設定しても
       後から設定した分が先の分を上書きしないこと(それぞれ別の通知が
       届くこと)。端末再起動後も再登録されること。
    4. **シートの書式表示**: デスクトップで太字/斜体/下線/取り消し線/
       文字色/背景色/配置/フォントサイズを設定したセルが、「📊 表」でも
       同じ装飾で表示されること(紙面は白背景・黒文字が基本、ダーク
       モードでも変わらないこと)。
    5. **列幅・行高・結合**: デスクトップで列幅・行高を変更、またはセルを
       結合すると、Android 側にも近い比率で幅・高さが反映され、結合
       セルが 1 マスとして表示されること(結合セル内の値は左上セルの
       タップで編集できること)。
    6. **目盛線**: デスクトップで目盛線を OFF にしたシートは、Android でも
       罫線がごく薄く(実質見えない)表示に変わること。

110. **情報共有スイート改修(M6 H、ADR-0055、デスクトップ)**: 手順
    103〜108 の再確認に加えて
    1. **メンション**: チャット入力でも `@` でサジェスト(全メンバー +
       @All がスクロールで全部出る。ライトモードでも読める)こと。
       `@All` 入りコメント/チャットで全員に通知が出ること。メンション・
       自メモへのコメントを受けた側は、OS 通知に加えて**チャットに
       お知らせ行**(📝 カード付き)が入ること。
    2. **コメント欄**: コメントが多くてもメモ本文が見えること(コメントは
       独立スクロール)。
    3. **メモ**: ⏰(リマインダー)がメモから消えていること。
    4. **スケジュール**: 土曜青・日曜/祝日赤 + 祝日名バッジ。予定に参加
       メンバーを設定 → 👥n 表示 + 「自分の予定」フィルタが効くこと。
       予定に「5 分前」等のリマインダーを複数設定でき、時刻に通知される
       こと。
    5. **シート(入力)**: 「abc」がそのまま入り、日本語(IME)入力が
       できること。表がダークモードでも白紙面で、画面いっぱいに使える
       こと。
    6. **シート(書式)**: 複数セルを選択して太字・色・背景・罫線・配置・
       サイズを適用 → 他の端末にも反映。セル結合/解除、列幅・行高の
       ドラッグ変更、目盛線 OFF、ウインドウ枠の固定が効くこと。
    7. **シート(操作)**: 右クリックで独自メニュー(コピー/貼り付け/
       クリア/罫線/結合等)。Ctrl+F でシート内検索(n/m + 前後移動)。
       Ctrl+Z で自分の直近操作が戻ること(他人が先に変えたセルは競合
       通知でスキップ)。
    8. **プレゼンス**: 2 台で同じシートを開くと、相手の選択セルに枠色 +
       名前バッジが出て、移動に追従すること(切断で約 10 秒後に消える)。
