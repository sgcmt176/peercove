...
| **M2** | daemon 分離 + Tauri/React フル UI + インストーラ(仕様: `peercove-m2-handoff.md` v1.0 確定) | ⬅️ **進行中。G1〜G6 実装済み(G1/G2 実機検証済み)。残り G7: インストーラ・サービス化** |
...
1. **C-1(GitHub + CI)を最初に**: 資産保全と、Opus 含む複数エージェント作業の
   品質ゲートになる。半日サイズ。**まだ未着手**(リモート未設定のまま歴史が伸びている)
2. ~~A-1(M2 要件確認)~~ → 完了。A-2 / A-3 も完了
3. **A-4 = M2-G7 は前後半に分割済み**。前半(G7a: サービス化、ADR-0010)は
   実装済みで Session 0 PoC の実機検証待ち。後半(G7b: MSI / deb / ZIP)は
   Opus 担当 — 指示書は `docs/peercove-g7-packaging-handoff.md`
