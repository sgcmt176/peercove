...
- **最終更新**: 2026-07-12(M3-14b 実装 + 検証 FB 修正(メンバー側のレコード一覧表示)、実機検証待ち。M3-14c は Opus 担当 — 指示書 `docs/peercove-m3-14c-instructions-for-opus.md`。M3-12 も Opus 担当予定)
...
| M3-14 | DNS 拡張トラック | 依頼者要望(2026-07-12)の DNS 機能群。**14a(表示名/DNS 名の分離・固定化)は完了(実機検証済み 2026-07-12、ADR-0021)。14b(エイリアス・サービス名・サブドメイン・LAN 機器レコード)は実装済み(2026-07-12、ADR-0022)、実機検証待ち**。以降 14c(サービス情報 UI、Opus 候補)→ 14d(逆引き)/14f(ワイルドカード、Opus 候補)→ 14e(状態連動)→ 14g(短縮名) | ★〜★★★★ | 14a が他すべての前提 |
