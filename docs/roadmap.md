...
| A-5 | コード署名 | **見送り(2026-07-23 依頼者判断)**。証明書の費用が発生するため対応しない。リリースは未署名のまま(自動アップデート適用も引き続き保留) | ★★ | 見送り |
...
次期テーマ: **ホスト端末以外からのメンバー招待** — 設計 = ADR-0048
(2026-07-20 依頼者承認)、**実装済み(2026-07-21、実機検証待ち =
verification.md 手順 85〜87)**。案 A(招待依頼のリレー)+ 端末指名の
2 段権限 + invited_by 記録 + メンバー詳細ページ(デスクトップ + Android)。

### トラック F: M5 — メモ帳(個人 + 共有、2026-07-21 要件受領)

依頼者要件の原文は `docs/history/memo-feature-requirements.md`、技術判断は
ADR-0049(SQLite 採用・`peercove-memo` crate に一本化・単一編集者ロック +
リビジョン・member_id 権限・平文保存の明示)。

| Phase | 内容 | 状態 |
|---|---|---|
| F-1 | 個人メモ(Markdown・チェックリスト・フォルダー/タグ・FTS 検索・ゴミ箱・txt 入出力)。デスクトップ = デーモン所有 + IPC、Android = Rust コア | **完了(実機検証済み 2026-07-22。verification.md 手順 88〜89)** |
| F-2 | 共有メモ(ホスト正本 DB・権限・編集ロック・リアルタイム閲覧・リビジョン・読み取りキャッシュ) | **完了(実機検証済み 2026-07-22。verification.md 手順 90〜93)** |
| F-3 | 管理(変更履歴・差分・復元・容量制限・ゴミ箱自動削除・バックアップ ADR-0034 / 診断センター統合・個人→共有変換) | **完了(実機検証済み 2026-07-23。verification.md 手順 94〜99)** |
| F-4 | グループ対象の権限(ADR-0051: グループのホスト同期 + 個別 > グループ > 全体) | **実装済み(2026-07-24、実機検証待ち = verification.md 手順 102)** |
| F-5 | メモ間リンク・バックリンク・コメント・メンション・リマインダー + 共有ハブ + チャットの @memo: リンクカード(ADR-0052。**CRDT 同時編集は見送り確定** = 2026-07-23 依頼者判断) | **実装済み(2026-07-24、実機検証待ち = verification.md 手順 103〜106)** |

### トラック G: M6 — 情報共有スイート(2026-07-24 依頼者指示)

共有ハブ(ADR-0052)に載せる情報共有機能群。共有メモの基盤(ホスト正本
DB・配信・キャッシュ・バックアップ/診断・@種別:id カード)を転用する。

| # | 機能 | 概要 | 状態 |
|---|---|---|---|
| G-1 | スケジュール表 | ADR-0053。月カレンダー + 予定 CRUD(全員追加可、編集は作成者 + ホスト、revision CAS)+ @schedule: カード | **実装済み(2026-07-24、実機検証待ち = verification.md 手順 107)** |
| G-2 | Excel ライク表 | ADR-0054。セル単位 revision CAS の共有シート(全員編集可・固定アドレス・数式なし = V1)+ @sheet: カード | **実装済み(2026-07-24、実機検証待ち = verification.md 手順 108)** |
| G-3 | ホワイトボード | 追記型ストローク共有(需要確認後) | 構想 |
| H | 検証フィードバック改修 | ADR-0055。メンション拡充(チャット/@All/チャット通知行)・コメント欄改善・メモのリマインダー→スケジュール移設・祝日カレンダー・参加メンバー・シート v2(書式/結合/列幅/固定枠/右クリック/プレゼンス/検索/Undo)+ シート入力バグ修正 | **実装済み(2026-07-24、実機検証待ち = verification.md 手順 109〜110)** |
| — | 通話・画面共有 | 内蔵化は見送り方針(トンネル越しに既存ツール(RDP / Parsec / Moonlight 等)を使う案内を優先。ADR-0052 検討時の判断) | 構想 |
...
- **メンバー側の設定変更**: 解消済み(2026-07-23)。member.toml も従来から
  5 秒 tick で再読込されており(direct・受信上限は動的)、残っていた
  待受ポート・MTU・接続先も変更検知で自動的にトンネルを作り直して反映する
  ようにした(鍵ローテーションの Restart 経路に相乗り。ホストの UPnP
  リースも張り直す)。残る既知の小穴: ホストのポート変更直後、IPC status に
  出る外部エンドポイント表示がデーモン再起動まで古いままになることがある
  (表示のみ。実際のポート開放・接続には影響しない)
