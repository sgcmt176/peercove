...
- **最終更新**: 2026-07-10(M3-5 まで実機検証済み。M3-13 チャットを追加)
...
| M3-7 | サブネットルーター | **7a 完了(実機検証済み 2026-07-10、Windows ホスト + Linux ルーター役 + Docker 疑似 LAN)**(ADR-0014)。**7b(UI)実装済み(2026-07-10)実機検証待ち**(メンバー行の 🖧 で編集 + CIDR バッジ)。host.toml `[[peer]].subnets` が正本 → 台帳配布 → AllowedIPs/OS ルート。ルーター役は V1 Linux 限定(転送 + SNAT 自動設定)。7b = UI(サブネット編集 + バッジ) | ★★★ | ACL より先に |
| M3-8 | ~~Wake-on-LAN~~ | **不要**(2026-07-10 依頼者判断で対象外) | — | —  |
| M3-9 | ファイル送信 | メンバーへのファイル転送(Taildrop 相当)。トンネル内 TCP | ★★★ | 直接通信後が効果的(リレー経由でも動作)。チャット(M3-13)と基盤を共有する前提で設計 |
...
| M3-13 | チャット | メンバー同士のメッセージ送受(LINE 風の 1:1 / ネットワーク全体)。対象は VPN 内メンバーのみ。トンネル内通信で外部サーバー不要。履歴はローカル保存(2026-07-10 依頼者追加) | ★★★ | M3-9 と基盤(メンバー間のトンネル内メッセージング)を共有。M3-9 の直後に実装、M3-12 より前 |
