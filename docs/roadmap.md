...
| M3-2 | 直接通信 B-1 | ✅ 実装済み(2026-07-10)。外部エンドポイント + 観測経過秒を台帳配布(オンラインのみ)+ Windows デバイスの最長一致修正(ADR-0013) | ★★ | —  |
| M3-3 | 直接通信 B-2 | ✅ 実装済み(2026-07-10)**実機検証待ち**。台帳駆動の暗黙パンチング。成功時は /32 直接ピア。`direct` 設定フラグ + 鮮度ガード + タイムアウト/クールダウン(ADR-0013) | ★★★★★ | —  |
| M3-4 | 直接通信 B-3 | ✅ 実装済み(2026-07-10)**実機検証待ち**。指数バックオフ(5 分〜1 時間)+ 経路バッジ(直接/中継/確立中)+ 外部 IP 共有の UI 説明文(ADR-0013) | ★★★★ | —  |
