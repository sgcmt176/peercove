...
| **M2** | daemon 分離 + Tauri/React フル UI + インストーラ(仕様: `peercove-m2-handoff.md`) | ⬅️ **handoff ドラフトのレビュー待ち** |
...
└─ peercove-ui/              # M2 デスクトップ UI(Tauri 2 + React)
    ...
    ├─ src/                  # React。ipc.ts(UI 用の型)、App.tsx
...
| # | タスク | 概要 | 難易度 | 担当目安 |
|---|---|---|---|---|
| A-1 | M2 要件の確定 | `peercove-m2-handoff.md` ドラフト作成済み(**依頼者レビュー待ち**。§9 の未確定事項 Q1〜Q6 に回答で確定) | — | **依頼者**(+相談) |
| A-2 | daemon 分離 | peercove-poc を「常駐デーモン + 制御 CLI」に分割(UI の前提)。ローカル IPC(名前付きパイプ/UDS)の設計 | ★★★★ | Fable |
| A-3 | Tauri + React UI | 接続状態・members 一覧・invite(QR 表示)・join(貼り付け/カメラ)・remove の GUI | ★★★ | Opus 可(設計レビューは Fable) |
| A-4 | インストーラ・自動起動 | Windows MSI / Linux パッケージ、wintun.dll 同梱の扱い(ライセンス確認) | ★★★ | Opus 可 |
| A-5 | コード署名 | 証明書の取得・署名パイプライン | ★★ | 依頼者(手続き)+ Opus |
...
| # | タスク | 概要 | 難易度 | 担当目安 |
|---|---|---|---|---|
| C-1 | GitHub リポジトリ + CI | リモート未設定のまま歴史が長くなっている。GitHub Actions で fmt/clippy/test + zigbuild(Linux) | ★★ | Opus 可 |
...
| C-5 | status のリアルタイム化 | ステータスファイル(5 秒)をコントロールチャネル/IPC に統合(A-2 とセットが自然) | ★★★ | A-2 に合流 |

### 推奨順序

1. **C-1(GitHub + CI)を最初に**: 資産保全と、Opus 含む複数エージェント作業の
   品質ゲートになる。半日サイズ
2. **A-1(M2 要件確認)を並行で**: 上位文書の UI 要件を依頼者が確認し、
   M2 handoff を作る(相談ベースで叩き台作成は手伝える)
3. その後、**A(プロダクト化)を主線**に、B(直接通信)は A-2 の daemon 分離が
   終わってから着手するのが手戻りが少ない(パンチングの調停はデーモン常駐が前提に
   なるため)。ゲーム用途の体感を早く上げたいなら B を先にする判断もあり
...
- git リモート未設定(→ トラック C-1)。現状はローカルリポジトリのみ
