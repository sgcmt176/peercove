...
- **最終更新**: 2026-07-11(M3-13a/b 実機検証済み。13c グループ・13d チャット内ファイル + D&D 実装済み、実機検証待ち)
...
| M3-4 | 直接通信 B-3 | ✅ 完了(実機検証済み 2026-07-10)。指数バックオフ(5 分〜1 時間)+ 経路バッジ(直接/中継/確立中)+ 外部 IP 共有の UI 説明文(ADR-0013) | ★★★★ | —  |
...
| M3-10 | アクセス制御(ACL) | メンバー間通信の許可/禁止をホストが制御 | ★★★★ | **B 完了後**(リレー経路と直接経路の両方に効かせるため) |
| M3-11 | 鍵ローテーション | C-4 を合流。コントロールチャネル経由でメンバー鍵更新 | ★★★★ | B 完了後(プロトコル変更の競合回避) |
| M3-12 | 自動アップデート通知 | GitHub Releases を見て新版を通知 | ★★ | **最後**(依頼者指定)。自動適用はコード署名(M4)後 |
| M3-13 | チャット | メンバー同士のメッセージ送受(LINE 風の 1:1 / **任意グループ** / ネットワーク全体。**チャット内ファイル送信 + ドラッグ&ドロップ対応** — 2026-07-11 依頼者追加)(ADR-0016)。対象は VPN 内メンバーのみ。トンネル内通信で外部サーバー不要。履歴はローカル保存。実装は 13a(基盤)→ 13b(UI)→ 13c(グループ)→ 13d(ファイル + D&D)の 4 ゴール。**13a・13b 完了(実機検証済み 2026-07-11)。13c(グループ)と 13d(チャット内ファイル + D&D)実装済み + 検証フィードバック 1 巡目対応済み(2026-07-11: グループ同期を ack ベース再送に修正・グループ操作のお知らせ表示・画像/動画のインラインプレビュー + クリック拡大)、2 巡目対応済み(2026-07-11: Linux の受信ファイルプレビュー不可 = asset スコープの隠しディレクトリ既定を解除・ダイアログ内ドラッグ選択での誤クローズ修正)、**追加要望 13e 実装済み(2026-07-11: テキストファイルのプレビュー・受信失敗のお知らせ・メンバー一覧のファイル送信をボタン + 宛先チェックボックス化・OS 通知のオン/オフ設定・URL のリンク化・リンクプレビュー — ADR-0017)**、実機検証待ち** | ★★★ | M3-9 と基盤(メンバー間のトンネル内メッセージング)を共有。M3-12 より前 |
