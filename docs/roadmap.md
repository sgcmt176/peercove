...
3. ~~チャット送信失敗の再送キュー(自動再送・手動再送・取消・重複防止)~~
   → **実装済み(2026-07-20、モバイル)**。残: キューの永続化(アプリ再起動を
   跨ぐ自動再送。現在は失敗表示から手動再送で継続できる)・デスクトップ側の送信キュー
4. ファイル転送の中断再開(チャンク化 + ハッシュ検証。
   デスクトップとのプロトコル変更が必要)
5. 受信ファイルの「開く」「共有」(FileProvider)
6. アプリ別 VPN(VpnService.Builder の allow/disallow アプリ)
7. 通信品質履歴の簡易版(RTT グラフ・切断タイムライン・回線切替履歴)
8. グループ管理の拡充(改名・メンバー編集・退出をスマホから)
9. チャット検索・ファイル一覧・ストレージ管理
10. ホスト側の端末管理(hello に platform 任意フィールドを追加して
    OS バッジ・端末単位の失効など。後方互換必須)
