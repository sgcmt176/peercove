...
| M3-7 | サブネットルーター | **7a 実装済み(2026-07-10)実機検証待ち**(ADR-0014 承認済み)。host.toml `[[peer]].subnets` が正本 → 台帳配布 → AllowedIPs/OS ルート。ルーター役は V1 Linux 限定(転送 + SNAT 自動設定)。7b = UI(サブネット編集 + バッジ) | ★★★ | ACL より先に |
