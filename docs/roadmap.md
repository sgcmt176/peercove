...
- **最終更新**: 2026-07-14(M3-19 表示名変更をメンバー一覧へ一元化 + メンバー本人の表示名変更(`set_display_name` 制御メッセージ)+ 設定画面での表示名/DNS名編集を削除(ADR-0027)。メンバーの設定画面表示名が反映されない不具合も修正。直前は M3-18 バイナリ改名(ADR-0026)。M3-12 は Opus 担当予定)
...
| M3-14 | DNS 拡張トラック | 依頼者要望(2026-07-12)の DNS 機能群。**14a(表示名/DNS 名の分離・固定化)は完了(実機検証済み 2026-07-12、ADR-0021)。14b(エイリアス・サービス名・サブドメイン・LAN 機器レコード、ADR-0022)と 14c(スキーム/ポートのサービス情報・URL コピー、ADR-0023)は実装済み(2026-07-12)、実機検証待ち**。以降 14d(逆引き)/14f(ワイルドカード、Opus 候補)→ 14e(状態連動)→ 14g(短縮名) | ★〜★★★★ | 14a が他すべての前提 |
