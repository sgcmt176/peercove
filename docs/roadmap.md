...
| Phase | 内容 | 状態 |
|---|---|---|
| F-1 | 個人メモ(Markdown・チェックリスト・フォルダー/タグ・FTS 検索・ゴミ箱・txt 入出力)。デスクトップ = デーモン所有 + IPC、Android = Rust コア | **実装済み(2026-07-21、実機検証待ち = verification.md 手順 88〜89)** |
| F-2 | 共有メモ(ホスト正本 DB・権限・編集ロック・リアルタイム閲覧・リビジョン・読み取りキャッシュ) | **実装済み(2026-07-21、実機検証待ち = verification.md 手順 90〜93)** |
| F-3 | 管理(変更履歴・差分・復元・容量制限・ゴミ箱自動削除・バックアップ ADR-0034 / 診断センター統合・個人→共有変換) | **実装済み(2026-07-22、実機検証待ち = verification.md 手順 94〜99)** |
