...
| M3-6 | UI デザインパス | ✅ 実装済み(2026-07-10)**実機検証待ち**。テーマ切替(システム/ライト/ダーク)・メンバー色分けアバター(公開鍵から決定的)・転送量/RTT スパークライン・トレイからのネットワーク別接続/切断。詳細画面をタブ構成にし、**ファイル送信(M3-9)・チャット(M3-13)はタブ追加で収まる**(2026-07-10 依頼者指定) | ★★ | なし |
