...
- **最終更新**: 2026-07-12(M3-11 実機検証済み。次は DNS 拡張トラック(表示名/DNS 名分離ほか、§4 M3-14 参照)。M3-12 は Opus 担当予定)
...
| M3-14 | DNS 拡張トラック | 依頼者要望(2026-07-12)の DNS 機能群。表示名/DNS 名の分離を基盤に、エイリアス・サービス名・逆引き・短縮名などを段階実装(内訳は依頼者と合意した分割による) | ★〜★★★★ | M3-14a(分離・固定化)が他すべての前提 |
