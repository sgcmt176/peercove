...
| M3-13 | チャット | メンバー同士のメッセージ送受(LINE 風の 1:1 / **任意グループ** / ネットワーク全体。**チャット内ファイル送信 + ドラッグ&ドロップ対応** — 2026-07-11 依頼者追加)(ADR-0016)。対象は VPN 内メンバーのみ。トンネル内通信で外部サーバー不要。履歴はローカル保存。実装は 13a(基盤)→ 13b(UI)→ 13c(グループ)→ 13d(ファイル + D&D)の 4 ゴール | ★★★ | M3-9 と基盤(メンバー間のトンネル内メッセージング)を共有。M3-12 より前 |
