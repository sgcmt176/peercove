...
依頼者が選定した 9 機能。番号は推奨実装順(依存関係を考慮)。
1 ゴールずつ「設計(必要なら ADR)→ 実装 → README → コミット → 実機検証依頼」で進める。

| # | 機能 | 概要 | 難易度 | 依存・備考 |
|---|---|---|---|---|
| M3-1 | DNS(内蔵リゾルバ) | `<名前>.peercove.internal` で相互アクセス。デーモン内蔵 DNS + OS のスプリット DNS 設定(ADR-0011) | ★★★★ | なし |
