...
| # | 機能 | 概要 | 難易度 | 依存・備考 |
|---|---|---|---|---|
| M3-0a | 複数ネットワーク: 設定配置 | ✅ 実装済み(2026-07-09)。networks/ + 自動移行 + ネットワーク名(init・トークン v2)(ADR-0012) | ★★★ | **実機検証待ち** |
| M3-0b | 複数ネットワーク: デーモン | ✅ 実装済み(2026-07-09)。多重トンネル + IPC 改訂 + CLI 追随 | ★★★★ | **実機検証待ち** |
| M3-0c | 複数ネットワーク: UI | ✅ 実装済み(2026-07-09)。カード一覧 + 個別接続/切断 + 追加/削除 | ★★★ | **実機検証待ち** |
| M3-1 | DNS(内蔵リゾルバ) | `<名前>.<ネットワーク名>.peercove.internal` で相互アクセス。デーモン内蔵 DNS + OS のスプリット DNS + カスタムレコード管理画面(ADR-0011) | ★★★★ | M3-0 |
| M3-2 | 直接通信 B-1 | ホストが学習済みの外部エンドポイントを台帳に載せて配布 | ★★ | なし |
| M3-3 | 直接通信 B-2 | コントロールチャネルでパンチング調停、成功時は /32 直接ピア | ★★★★★ | M3-2 |
| M3-4 | 直接通信 B-3 | 失敗時のホスト経由フォールバック + 経路の status/UI 表示 | ★★★★ | M3-3 |
| M3-5 | 小粒バッチ | ログイン時自動起動 + 招待ディープリンク(`peercove://join`) | ★★ | なし |
| M3-6 | UI デザインパス | ダークモード・メンバー色分け・転送量/RTT スパークライン・トレイ操作拡充 | ★★ | なし |
| M3-7 | サブネットルーター | メンバー背後の LAN(NAS 等)への経路広告と中継 | ★★★ | ACL より先に |
| M3-8 | Wake-on-LAN | 同 LAN の別メンバー経由でオフライン PC を起こす | ★★ | なし |
| M3-9 | ファイル送信 | メンバーへのファイル転送(Taildrop 相当)。トンネル内 TCP | ★★★ | 直接通信後が効果的(リレー経由でも動作) |
