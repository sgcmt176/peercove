...
| # | 機能 | 概要 | 難易度 | 依存・備考 |
|---|---|---|---|---|
| M3-0a | 複数ネットワーク: 設定配置 | networks/ ディレクトリ + 自動移行 + ネットワーク名(init・トークン拡張)(ADR-0012) | ★★★ | なし |
| M3-0b | 複数ネットワーク: デーモン | 多重トンネル化 + IPC 改訂 + CLI 追随 | ★★★★ | M3-0a |
| M3-0c | 複数ネットワーク: UI | ネットワーク一覧化(カード + 個別接続/切断) | ★★★ | M3-0b |
