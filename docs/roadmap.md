...
- **最終更新**: 2026-07-14(M3-22 招待期限・一回利用・参加承認隔離を実装(ADR-0031、実機検証待ち)。直前は M3-21 接続診断(ADR-0030)、M3-12 更新通知・版互換性(ADR-0029))
...
| M3-21 | 接続診断センター | ✅ **実装完了(2026-07-14、実機検証待ち)**(ADR-0030)。固定 ID の構造化診断、Unknown を含む総合判定、設定・秘密ファイル・トンネル・ハンドシェイク・到達証拠・DNS・版互換・削除状態・ACL の読み取り専用チェック。問題優先 UI、二段階 redact、保存前確認付き JSON/TXT ローカル出力 | ★★★ | M3-12 の版・capability 情報を利用 |
