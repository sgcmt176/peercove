...
| M3-9 | ファイル送信 | **9a(基盤 + CLI)実装済み(2026-07-10)実機検証待ち**(ADR-0015)。各デーモンが仮想 IP の TCP 51822 で待受け(台帳照合)、受信ボックス `networks/<net>.inbox/` に自動保存(SHA-256 検証)。IPC SendFile + status に進捗。9b = UI | ★★★ | チャット(M3-13)と基盤(トンネル内メッセージング)を共有 |
