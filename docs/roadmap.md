...
- **最終更新**: 2026-07-13(M3-16 UI 刷新 = 左サイドバー化・設定/DNS/サブネットのページ化・チャット入力の下部固定/可変高・1:1 チャット導線・DNS レコード UI 再構成 + ワイルドカード(ADR-0024)、実機検証中。次は転送先 CNAME(外部ドメイン対応、ADR-0025 予定)。M3-12 は Opus 担当予定)
