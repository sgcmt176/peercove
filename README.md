...
## サブネットルーター(M3-7a、CLI まで — UI は M3-7b)

あるメンバーの背後 LAN(NAS など、PeerCove を入れられない機器)へ、
全メンバーがトンネル越しに届くようにします(ADR-0014)。

- **ホストの設定が正本**です。ホストで次を実行すると台帳で全員に配布されます:
  ```bash
  peercove-poc subnet --config host.toml --name alice 192.168.10.0/24
  peercove-poc subnet --config host.toml --name alice          # 解除
  ```
