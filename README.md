...
- 設定、秘密ファイルの存在、Unix のファイル mode、インターフェース、ハンドシェイク、DNS、版互換、
  ホストからの削除、ACL を確認します。Windows ACL の詳細確認は現段階では判定不能です。
- 保存前に含有情報を確認し、構造化 JSON と同名の TXT をローカルへ保存できます。自動送信はしません。
- 秘密鍵、PSK、招待トークンはレポートへ含めません。ログはデーモンで再度 redact し、保存側の deny-list
  に一致した場合も保存を中止します。
...
4. メンバーを ACL で遮断、またはホストから削除して再診断し、それぞれの固定診断項目が警告／失敗に
   なること。
5. 「診断結果を保存」で確認文を読んで保存し、`.json` と `.txt` が同じ場所に生成されること。
6. 両ファイルを検索し、秘密鍵本文、PSK、招待トークンが含まれないこと。設定パスや仮想 IP など
   共有したくない情報が含まれうる旨も確認画面に表示されること。
7. Windows と Ubuntu で同じ状況の check ID と意味が一致し、Ubuntu では group/other に読取権限を
   付けた秘密ファイルが警告になること。
...
- v3 の join は端末上で device ID を生成します。同じトークンを別端末へコピーした場合、最初に参加した
  device ID だけが再接続でき、後続端末は拒否されます。同時接続でも先行セッションを維持します。
...
3. 同じ token v3 を別ディレクトリまたは端末 B で join し、端末 A の参加後は B が台帳を取得できず、
   ホストログに「別の端末で既に使用」と出ること。A の接続は切れないこと。
