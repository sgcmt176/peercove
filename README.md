...
- **ホストの設定が正本**です。**ホストのマシンで host.toml に対して**実行します
  ...
  .\peercove-poc.exe subnet --config "$env:APPDATA\app.peercove.desktop\networks\<名前>\host.toml" --name alice 192.168.10.0/24
...
- 注意: `subnets` を書いた host.toml は**旧バージョンの peercove-poc では
...
2. ホスト: `peercove-poc subnet --config <host.toml> --name <ルーター役> 192.168.10.0/24`
...
- **受信サイズ上限は既定 100 MB**です。**受け取る側**の設定
  ...
  旧バージョンの peercove-poc では読めません(明示エラー)
- CLI(デーモン経由で送信し、進捗を表示します):
  ```bash
  # Windows の例(PowerShell)。--to は表示名または仮想 IP
  .\peercove-poc.exe send-file --config "$env:APPDATA\app.peercove.desktop\networks\<名前>\host.toml" --to alice .\photo.jpg
  ```
- 相手が旧バージョン(リスナーなし)の場合は「相手に接続できません」に
  なります
- **UI から**(M3-9b): メンバー行の 📤 でファイルを選んで送信。進捗は
  「受信」タブに出ます。受け取った側は OS 通知 +「受信」タブの
...
1. 送信側: `peercove-poc send-file --config <設定> --to <相手の表示名> <ファイル>`
...
1. 送信: メンバー行の 📤 → ファイルを選ぶ → 「受信」タブに進捗バーが出て
   「完了」になること
...
- CLI(デーモン経由):
  ```bash
  # 1:1(--to は表示名または仮想 IP)
  .\peercove-poc.exe chat --config <設定> --to alice "こんにちは"
  # ネットワーク全体へ
  .\peercove-poc.exe chat --config <設定> --all "全員へ"
  # グループへ(グループ名または ID。作成は UI から)
  .\peercove-poc.exe chat --config <設定> --group 開発チーム "グループへ"
  # 履歴の表示(--follow で新着を待ち続ける)
  .\peercove-poc.exe chat-log --config <設定> --follow
  ```
...
- グループの情報は**サーバーに置かず、メンバー同士が直接配り合います**
  ...
  オフラインだったメンバーには、そのメンバーがオンラインに戻ったのを
  他のメンバーが検知したときに自動で送り直されます
...
3. **ドラッグ&ドロップ**: エクスプローラーからファイルを会話画面へ
   ドラッグ → 点線の受け入れ表示が出る → ドロップ → 確認ダイアログ →
   送信されること。複数ファイルを一度にドロップしても全部送られること
4. **グループ宛ファイル**: グループの会話にドロップ → グループの
   オンラインメンバー全員に届くこと(グループ外のメンバーには届かない)
5. **オフライン宛**: オフラインの相手の会話では 📎 が無効。ドロップすると
   「オフラインのメンバーには送れません」の案内が出て送信されないこと
6. **大きめファイル**: 数百 MB のファイルで、送受両方のバブルに進捗バーが
