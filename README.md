...
20. ウィンドウを閉じた状態でメンバーが接続 → **「メンバーが参加しました」**の
    OS 通知が出ること。切断すると「メンバーが切断しました」が出ること
21. 通知はメンバー側でも出ること(ほかのメンバーの参加・切断が見える)
22. トレイの「終了」でアプリが終了し、**トンネルは切れないこと**
    (デーモンが維持している。`peercove-poc daemon status` で確認)
23. UI を起動し直しても、既に接続済みのメンバーについて**通知が鳴らない**こと
    (初回の状態は基準にするだけで通知しない)

> **Windows のトースト通知について**: `npm run tauri dev` で動かしている間は、
> アプリが AppUserModelID を登録していないため通知が出ないことがあります。
> その場合は M2-G7(インストーラ)の後に再確認してください。トレイ常駐と
> それ以外の項目は dev のままで確認できます。
...
8. メンバー機の UI の「参加する」にトークンを貼り付けて **「参加する」**
