...
- 注意: ローテーション後の member.toml(`key_source` あり)は**旧バージョンの
  peercove-poc では読めません**(明示エラー)。全マシンのデーモンを先に
...
3. **通信の回復**: 更新後、ping・チャット・ファイル送信が従来どおり動き、
   直接通信できる環境では経路バッジが数分以内に「直接」へ戻ること
...
2. **遮断**: ホスト UI のメンバータブ →「🚦 通信制御」→ A ⇔ B の組を
   「遮断する」にチェック → 約 5 秒待つ
