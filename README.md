...
ホストのメンバー画面にある「通信制御」では、上から順に評価するルールを管理できます。
...
2. A→BだけUDPのテスト用ポートを許可し、その後にB宛ANY拒否を置く。UDPテストは指定方向・
   指定ポートだけ成功し、逆方向、TCP、範囲外ポート、ICMPが拒否されること。
...
1. ホストとメンバーを接続し、1分以上待って「品質」を開く。相手を選択でき、RTT の線、損失の棒、
   経路帯、下部の数値表が同じ時刻・値を示すこと。
