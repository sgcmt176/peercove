...
- **M3-4(未実装)**: 再試行の指数バックオフと、経路(直接/中継)の UI 表示

直接通信を使いたくないマシンでは、設定ファイル(member.toml)の
`[interface]` に `direct = false` を書くと常にホスト経由になります
(既定は `true`。将来 UI の設定画面からも切り替えられるようにする予定)。
...
1. 3 台で同じネットワークに接続する
2. メンバー A のログ(UI のログビュー / `journalctl -u peercove-daemon`)に
   「直接接続を試行します(…)」→「直接通信を確立しました(…)」が
   出ること(B 側にも同様に出る)
...
5. RTT の改善(任意): メンバー一覧の RTT 表示が中継時代より下がること
   (ホストが地理的に遠い構成ほど顕著)
6. フォールバック: B を切断 → A のログに「台帳から外れたため直接ピアを
   解除します」が出て、B のピア行が消えること。B を再接続すると
   自動的にまた確立すること
7. 無効化: A の member.toml の `[interface]` に `direct = false` を追記
   (稼働中でも約 10 秒で反映)→ A のログに「解除します」が出て、
   以後 B と直接ピアを張らないこと。ping はホスト経由で通り続けること。
   行を消せば(= 既定 true)再び直接化されること
8. パンチ不可の NAT(例: キャリア回線同士)の場合: 追加から 30 秒で
   「直接接続がタイムアウトしました」が出て、ping はホスト経由で
   通り続けること(通信は途切れない)
