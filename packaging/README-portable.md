...
- `peercove(.exe)` … CLI + デーモン本体
- `peercove-ui(.exe)` … デスクトップ UI(GUI)
...
.\peercove.exe daemon service-install      # 登録 + 起動 + ファイアウォール許可

# Linux: root
sudo ./peercove daemon service-install
```

その後 `peercove-ui`(UI)を**通常権限で**起動します。やめるときは:

```
# Windows(管理者) / Linux(root)
peercove daemon service-uninstall          # 停止 + 登録解除 + 残骸の片付け
```
...
別ターミナル(通常権限)で `peercove-ui`(UI)を起動します。終了は Ctrl+C。
...
- Windows は `peercove.exe` と同じフォルダに `wintun.dll` が必要です
